...
/* Health tracking */
static uint32_t s_active_samples = 0; /* Samples above activity threshold */
#define STEP_LENGTH_CM 72             /* Average stride ~72 cm */
#define CALS_PER_STEP 0.04f           /* ~0.04 kcal per step (walking) */
#define ACTIVE_THRESH 1.15f           /* Accel magnitude > 1.15g = moving */
...
{
    uint8_t buf[12];
    if (i2c_read_buff(s_imu_dev, REG_AX_L, buf, 12) != ESP_OK)
        return false;
...
{
    health_data_t h;
    h.steps = s_step_count;
    h.distance_m = (s_step_count * STEP_LENGTH_CM) / 100;
    h.calories = (uint32_t)(s_step_count * CALS_PER_STEP);
    /* 50 Hz sample rate, active_samples / (50*60) = active minutes */
    h.active_min = s_active_samples / (50 * 60);
    return h;
}
